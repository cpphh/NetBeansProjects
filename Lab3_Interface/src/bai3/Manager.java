...
public class Manager {
	...
	int n=0;
        /*
	StudenAptech[] std=null;
	StudenMark[] stdmark = null;
	do {
	...
	System.out.println("4 Tim thong tin diem cua sinh vien theo id");
		...
		case 1:
			...
			std=new StudenAptech[m];
			for (int i = 0; i < m; i++) {
				std[i]=new StudenAptech();
				int dem=0;
				do {
	...
	if(std[i].getStuId().equals(std[j].getStuId())&&std!=null) {
		...
		case 2:
			...
			stdmark=new StudenMark[k];
			for (int i = 0; i < stdmark.length; i++) {
				stdmark[i]=new StudenMark();
				int dem=0;
				do {
	...
	System.out.println("can phai nhap thong tin cho sinh vien truoc");
			}
			break;
		case 3:
			if(std==null) {
